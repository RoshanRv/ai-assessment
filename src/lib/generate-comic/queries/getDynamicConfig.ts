...
export async function getDynamicConfig(): Promise<DynamicConfig> {
  ...
  const config = {
    ...
    oauthRedirectUrl: getValidString(
      process.env.HUGGING_FACE_OAUTH_REDIRECT_URL,
      ""
    ),
    oauthScopes: "openid profile inference-api",
    enableHuggingFaceOAuth: getValidBoolean(
      process.env.ENABLE_HUGGING_FACE_OAUTH,
      false
    ),
    enableHuggingFaceOAuthWall: getValidBoolean(
      process.env.ENABLE_HUGGING_FACE_OAUTH_WALL,
      false
    ),
  };
